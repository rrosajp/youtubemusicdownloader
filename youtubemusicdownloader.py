...
for i in range(len(linkInput)):
    downloadDetails = albumLinkCheck(linkInput[i])
    if downloadDetails[0] == False:
        print("Invalid input.")
        pass
    else:
        print("Fetching tags...")
        ...
        for i in range(len(trackName)):
            if platform.system() == "Windows":
                downloadDirectory = "\\\\?\\" + os.getcwd() + "\\YouTube Music\\" + albumArtistFixed[i] + "\\" + albumNameFixed[i] + "\\" + str(trackNumberFixed[i]) + " " + trackNameFixed[i] + ".m4a"
            else:
                downloadDirectory = os.getcwd() + "/YouTube Music/" + albumArtistFixed[i] + "/" + albumNameFixed[i] + "/" + str(trackNumberFixed[i]) + " " + trackNameFixed[i] + ".m4a"
            try:
