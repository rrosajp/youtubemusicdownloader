...
parser.add_argument(
    "url",
    help="Any valid YouTube Music album/playlist/track URL.",
    nargs="+",
)
parser.add_argument(
    "--o",
...
parser.add_argument(
    "--c", "--coverresolution", help='"max" or any valid number. Default is "1200".'
)
parser.add_argument(
    "--e",
    "--excludetags",
    default="",
    help='Any valid tag ("album", "albumartist", "artist", "artwork", "lyrics", "rating", "totaltracks", "tracknumber", "tracktitle" and "year") separated by comma with no spaces.',
)
parser.add_argument(
    "--s",
    "--savecover",
...
url = args.url
opus = args.o
exclude_tags_preferences = args.e
save_cover = args.s
...
def url_check(url):
    if re.search(r"watch\?v=\S\S\S\S\S\S\S\S\S\S\S", url) != None:
        track_video_id = re.search(r"watch\?v=\S\S\S\S\S\S\S\S\S\S\S", url).group(0)[8:]
    ...
    if (
        re.search(
            r"playlist\?list=OLAK5uy_\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S",
            url,
        )
        != None
    ):
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            album_playlist_details = ydl.extract_info(
                "https://music.youtube.com/playlist?list=" + album_playlist_id,
                download=False,
            )
        track_video_id = []
        for a in range(len(album_playlist_details["entries"])):
            if album_details["tracks"][a]["videoId"] == None:
                ...
                track_video_id.append(album_playlist_details["entries"][a]["id"])
        track_video_id[0]
        return track_video_id
    if (
        re.search(
            r"playlist\?list=\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S\S",
            url,
        )
        != None
    ):
        ...
        for a in range(playlis_details["trackCount"]):
            try:
                ...
                if playlis_details["tracks"][a]["videoId"] == None:
                    ...
                    track_video_id.append(playlis_details["tracks"][a]["videoId"])
            except:
                pass
        track_video_id[0]
        return track_video_id
...
def fetch_tags(track_video_id, cover_resolution):
    ...
    if "w60-h60-s" in track_album_details["thumbnails"][0]["url"]:
        track_album_cover = requests.get(
            track_album_details["thumbnails"][0]["url"].replace(
                "w60-h60-s", "w" + cover_resolution
            )
        ).content
    else:
        track_album_cover = requests.get(
            track_album_details["thumbnails"][0]["url"].replace(
                "w60-h60", "w" + cover_resolution
            )
        ).content
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        track_album_playlist_details = ydl.extract_info(
            ...
            + track_album_details["audioPlaylistId"],
            download=False,
        )
    ...
    for a in range(len(illegal_characters)):
        ...
        track_artist_fixed = track_artist_fixed.replace(illegal_characters[a], "_")
    ...
    return {
        "trackAlbumArtist": track_album_artist,
...
def get_download_preferences(opus, track_tags):
    if opus == True:
    ...
    if platform.system() == "Windows":
        ...
        current_directory = os.getcwd()
        slash = "/"
    track_download_directory = (
        current_directory
        + slash
        + "YouTube Music"
        + slash
        + track_tags["trackAlbumArtistFixed"]
        + slash
        + track_tags["trackAlbumNameFixed"]
        + slash
        + track_tags["trackNumberFixed"]
        + " "
        + track_tags["trackNameFixed"]
        + download_extension
    )
    cover_download_directory = (
        current_directory
        + slash
        + "YouTube Music"
        + slash
    ...
    return {
        "trackDownloadDirectory": track_download_directory,
        "coverDownloadDirectory": cover_download_directory,
...
def download(download_preferences, track_tags):
    ydl_opts = {
        "format": download_preferences["downloadFormat"],
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ydl.download("https://music.youtube.com/watch?v=" + track_tags["trackVideoId"])
...
def apply_tags(download_preferences, exclude_tags, track_tags):
    if exclude_tags["all"] != True:
        if download_preferences["downloadFormat"] == "251":
            tags = music_tag.load_file(download_preferences["trackDownloadDirectory"])
            ...
            if exclude_tags["artwork"] != True:
                tags["covr"] = [
                    MP4Cover(
                        track_tags["trackAlbumCover"], imageformat=MP4Cover.FORMAT_JPEG
                    )
                ]
            if exclude_tags["lyrics"] != True:
                if track_tags["trackLyrics"] != None:
            ...
            if exclude_tags["totaltracks"] != True:
                ...
                if exclude_tags["tracknumber"] != True:
                    tags["trkn"] = [
                        (track_tags["trackNumber"], track_tags["trackAlbumTotalTracks"])
                    ]
            if exclude_tags["tracknumber"] != True:
...
# Download cover.
def download_cover(save_cover, download_preferences, track_tags):
    if save_cover == True:
        with open(
            download_preferences["coverDownloadDirectory"] + "Cover.jpg", "wb"
        ) as cover_file:
            cover_file.write(track_tags["trackAlbumCover"])


# Set cover size.
if cover_resolution == "max":
    cover_resolution = "16383"
else:
    try:
        if (int(cover_resolution) < 0) or (int(cover_resolution) > 16383):
...
if exclude_tags_preferences != "":
    exclude_tags_preferences = exclude_tags_preferences.split(",")
    for a in range(len(exclude_tags_preferences)):
        exclude_tags[exclude_tags_preferences[a]] = True

# Start download.
track_video_id = []
print("\rChecking URL input...")
for a in range(len(url)):
    try:
        track_video_id += url_check(url[a])
...
for a in range(len(track_video_id)):
    try:
        print(
            "\rFetching tags (track "
            + str(a + 1)
            + " of "
            + str(len(track_video_id))
            + ")..."
        )
        track_tags = fetch_tags(track_video_id[a], cover_resolution)
        download_preferences = get_download_preferences(opus, track_tags)
        print(
            "\rDownloading "
            + '"'
            + track_tags["trackName"]
            + '"'
            + " (track "
            + str(a + 1)
            + " of "
        ...
        download_cover(save_cover, download_preferences, track_tags)
        print(
            "Download finished (track "
            + str(a + 1)
            + " of "
            + str(len(track_video_id))
            + ")!"
        ...
        error_count += 1
exit("All done (" + str(error_count) + " errors).")
